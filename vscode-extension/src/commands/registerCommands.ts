...
export function registerCostOptCommands(
  context: vscode.ExtensionContext,
  apiClient: CostOptApiClient,
  treeProvider: CostTreeDataProvider,
  statusBar: CostStatusBarItem,
  codeLensProvider: CostCodeLensProvider
): void {
  // 1. Show Cost Summary
  context.subscriptions.push(
    vscode.commands.registerCommand('costopt.showCostSummary', async () => {
      ...
      vscode.window.showInformationMessage(
        `CostOpt Summary — Today: $${forecast.spend_today.toFixed(2)} | Projected Monthly: $${forecast.projected_monthly.toFixed(2)} / $${forecast.budget.toFixed(2)} budget (${forecast.budget_remaining.toFixed(2)} remaining)`
      );
    })
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('costopt.showCurrentFileCost', async (filePathArg?: string) => {
      ...
      vscode.window.showInformationMessage(
        `CostOpt File Intelligence: ${fileStats.total_file_calls} calls recorded (${fileStats.line_stats.length} lines) | Total spend: $${fileStats.total_file_spend.toFixed(4)}`
      );
    })
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('costopt.showFeatureCosts', async () => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('costopt.showCostForecast', async () => {
      const forecast = await apiClient.getForecast();
      if (!forecast || !forecast.has_enough_data) {
      ...
      vscode.window.showInformationMessage(
        `CostOpt Monthly Forecast: $${forecast.projected_monthly.toFixed(2)} projected | Daily avg: $${forecast.daily_average.toFixed(2)} | Budget remaining: $${forecast.budget_remaining.toFixed(2)}`
      );
    })
  );
  ...
  // 5. Refresh Telemetry
  context.subscriptions.push(
    vscode.commands.registerCommand('costopt.refreshTelemetry', async () => {
